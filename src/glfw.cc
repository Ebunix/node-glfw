...
namespace glfw {
	...
	NAN_METHOD(GetVersion) {
		...
		arr->Set(Nan::GetCurrentContext(), JS_STR("major"), JS_INT(major)).ToChecked();
		arr->Set(Nan::GetCurrentContext(), JS_STR("minor"), JS_INT(minor)).ToChecked();
		arr->Set(Nan::GetCurrentContext(), JS_STR("rev"), JS_INT(rev)).ToChecked();
	...
	NAN_METHOD(SetTime) {
		Nan::HandleScope scope;
		double time = info[0]->NumberValue(Nan::GetCurrentContext()).ToChecked();
		glfwSetTime(time);
		return;
	}
	...
	NAN_METHOD(GetMonitors) {
		...
		for (i = 0; i < monitor_count; i++) {
			...
			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("is_primary"), JS_BOOL(monitors[i] == primary)).ToChecked();
			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("index"), JS_INT(i)).ToChecked();

			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("name"), JS_STR(glfwGetMonitorName(monitors[i]))).ToChecked();

			glfwGetMonitorPos(monitors[i], &xpos, &ypos);
			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("pos_x"), JS_INT(xpos)).ToChecked();
			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("pos_y"), JS_INT(ypos)).ToChecked();

			glfwGetMonitorPhysicalSize(monitors[i], &width, &height);
			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("width_mm"), JS_INT(width)).ToChecked();
			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("height_mm"), JS_INT(height)).ToChecked();

			mode = glfwGetVideoMode(monitors[i]);
			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("width"), JS_INT(mode->width)).ToChecked();
			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("height"), JS_INT(mode->height)).ToChecked();
			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("rate"), JS_INT(mode->refreshRate)).ToChecked();

			modes = glfwGetVideoModes(monitors[i], &mode_count);
			js_modes = Nan::New<Array>(mode_count);
			for (j = 0; j < mode_count; j++) {
				js_mode = Nan::New<Object>();
				js_mode->Set(Nan::GetCurrentContext(), JS_STR("width"), JS_INT(modes[j].width)).ToChecked();
				js_mode->Set(Nan::GetCurrentContext(), JS_STR("height"), JS_INT(modes[j].height)).ToChecked();
				js_mode->Set(Nan::GetCurrentContext(), JS_STR("rate"), JS_INT(modes[j].refreshRate)).ToChecked();
				// NOTE: Are color bits necessary?
				js_modes->Set(Nan::GetCurrentContext(), JS_INT(j), js_mode).ToChecked();
			}
			js_monitor->Set(Nan::GetCurrentContext(), JS_STR("modes"), js_modes).ToChecked();
			js_monitors->Set(Nan::GetCurrentContext(), JS_INT(i), js_monitor).ToChecked();
		}
	...
	void NAN_INLINE(CallEmitter(int argc, Local<Value> argv[])) {
		Nan::HandleScope scope;
		auto obj = glfw_events.Get(v8::Isolate::GetCurrent());
		if (Nan::New(glfw_events)->Has(Nan::GetCurrentContext(), JS_STR("emit")).ToChecked()) {
			Nan::New(glfw_events)->Get(Nan::GetCurrentContext(), JS_STR("emit")).ToLocalChecked().As<Function>();
			Nan::Callback callback(Nan::New(glfw_events)->Get(Nan::GetCurrentContext(), JS_STR("emit")).ToLocalChecked().As<Function>());
	...
	void APIENTRY windowPosCB(GLFWwindow *window, int xpos, int ypos) {
		...
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR("window_pos")).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("x"), JS_INT(xpos)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("y"), JS_INT(ypos)).ToChecked();
	...
	void APIENTRY windowSizeCB(GLFWwindow *window, int w, int h) {
		...
		Local<Array> evt = Nan::New<Array>(3);
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR("resize")).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("width"), JS_INT(w)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("height"), JS_INT(h)).ToChecked();
	...
	void APIENTRY windowFramebufferSizeCB(GLFWwindow *window, int w, int h) {
		...
		Local<Array> evt = Nan::New<Array>(3);
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR("framebuffer_resize")).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("width"), JS_INT(w)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("height"), JS_INT(h)).ToChecked();
	...
	void APIENTRY windowRefreshCB(GLFWwindow *window) {
		...
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR("refresh")).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("window"), JS_NUM((uint64_t)window)).ToChecked();
	...
	void APIENTRY windowIconifyCB(GLFWwindow *window, int iconified) {
		...
		Local<Array> evt = Nan::New<Array>(2);
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR("iconified")).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("iconified"), JS_BOOL(iconified)).ToChecked();
	...
	void APIENTRY windowFocusCB(GLFWwindow *window, int focused) {
		...
		Local<Array> evt = Nan::New<Array>(2);
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR("focused")).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("focused"), JS_BOOL(focused)).ToChecked();
	...
	void APIENTRY keyCB(GLFWwindow *window, int key, int scancode, int action, int mods) {
		...
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR(&actionNames[action << 3])).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("ctrlKey"), JS_BOOL(mods & GLFW_MOD_CONTROL)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("shiftKey"), JS_BOOL(mods & GLFW_MOD_SHIFT)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("altKey"), JS_BOOL(mods & GLFW_MOD_ALT)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("metaKey"), JS_BOOL(mods & GLFW_MOD_SUPER)).ToChecked();
		...
		evt->Set(Nan::GetCurrentContext(), JS_STR("keyCode"), JS_INT(key)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("charCode"), JS_INT(charCode)).ToChecked();
	...
	void APIENTRY cursorPosCB(GLFWwindow *window, double x, double y) {
		...
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR("mousemove")).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("x"), JS_NUM(x)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("y"), JS_NUM(y)).ToChecked();
	...
	void APIENTRY cursorEnterCB(GLFWwindow *window, int entered) {
		...
		Local<Array> evt = Nan::New<Array>(2);
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR("mouseenter")).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("entered"), JS_INT(entered)).ToChecked();
	...
	void APIENTRY mouseButtonCB(GLFWwindow *window, int button, int action,
															int mods) {
		Nan::HandleScope scope;
		Local<Array> evt = Nan::New<Array>(7);
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR(action ? "mousedown" : "mouseup")).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("button"), JS_INT(button)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("x"), JS_INT(lastX)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("y"), JS_INT(lastY)).ToChecked();
	...
	void APIENTRY scrollCB(GLFWwindow *window, double xoffset, double yoffset) {
		...
		Local<Array> evt = Nan::New<Array>(3);
		evt->Set(Nan::GetCurrentContext(), JS_STR("type"), JS_STR("mousewheel")).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("wheelDeltaX"),JS_NUM(xoffset * 120)).ToChecked();
		evt->Set(Nan::GetCurrentContext(), JS_STR("wheelDeltaY"),JS_NUM(yoffset * 120)).ToChecked();
	...
	NAN_METHOD(WindowHint) {
		Nan::HandleScope scope;
		int target = info[0]->Uint32Value(Nan::GetCurrentContext()).ToChecked();
		int hint = info[1]->Uint32Value(Nan::GetCurrentContext()).ToChecked();
	...
	NAN_METHOD(JoystickPresent) {
		Nan::HandleScope scope;
		int joy = info[0]->Uint32Value(Nan::GetCurrentContext()).ToChecked();
	...
	NAN_METHOD(glfw_CreateWindow) {
		Nan::HandleScope scope;
		int width = info[0]->Uint32Value(Nan::GetCurrentContext()).ToChecked();
		int height = info[1]->Uint32Value(Nan::GetCurrentContext()).ToChecked();
		String::Utf8Value str(Isolate::GetCurrent(), info[2]->ToString(Nan::GetCurrentContext()).ToLocalChecked());
		int monitor_idx = info[3]->Uint32Value(Nan::GetCurrentContext()).ToChecked();
		...
		glfw_events.Reset(info.This()->Get(Nan::GetCurrentContext(), JS_STR("events")).ToLocalChecked().As<v8::Object>());
	...
	NAN_METHOD(DestroyWindow) {
		Nan::HandleScope scope;
		uint64_t handle = info[0]->IntegerValue(Nan::GetCurrentContext()).ToChecked();
		if (handle) {
	...
	NAN_METHOD(SetWindowTitle) {
		Nan::HandleScope scope;
		uint64_t handle = info[0]->IntegerValue(Nan::GetCurrentContext()).ToChecked();
		String::Utf8Value str(Isolate::GetCurrent(),info[1]->ToString(Nan::GetCurrentContext()).ToLocalChecked());
		if (handle) {
			GLFWwindow *window = reinterpret_cast<GLFWwindow *>(handle);
	...
	NAN_METHOD(GetWindowSize) {
		...
		if (handle) {
			...
			arr->Set(Nan::GetCurrentContext(), JS_STR("width"), JS_INT(w)).ToChecked();
			arr->Set(Nan::GetCurrentContext(), JS_STR("height"), JS_INT(h)).ToChecked();
			info.GetReturnValue().Set(arr);
		}
		return;
	}
	...
	NAN_METHOD(SetWindowSize) {
		...
		if (handle) {
			GLFWwindow *window = reinterpret_cast<GLFWwindow *>(handle);
			glfwSetWindowSize(window, info[1]->Uint32Value(Nan::GetCurrentContext()).ToChecked(),info[2]->Uint32Value(Nan::GetCurrentContext()).ToChecked());
		}
		return;
	}
	...
	NAN_METHOD(SetWindowPos) {
		...
		if (handle) {
			GLFWwindow *window = reinterpret_cast<GLFWwindow *>(handle);
			glfwSetWindowPos(window, info[1]->Uint32Value(Nan::GetCurrentContext()).ToChecked(),info[2]->Uint32Value(Nan::GetCurrentContext()).ToChecked());
		}
		return;
	}
	...
	NAN_METHOD(GetWindowPos) {
		...
		if (handle) {
			...
			Local<Array> arr = Nan::New<Array>(2);
			arr->Set(Nan::GetCurrentContext(), JS_STR("x"), JS_INT(xpos)).ToChecked();
			arr->Set(Nan::GetCurrentContext(), JS_STR("y"), JS_INT(ypos)).ToChecked();
			info.GetReturnValue().Set(arr);
		}
		return;
	}
	...
	NAN_METHOD(GetFramebufferSize) {
		...
		if (handle) {
			...
			Local<Array> arr = Nan::New<Array>(2);
			arr->Set(Nan::GetCurrentContext(), JS_STR("width"), JS_INT(width)).ToChecked();
			arr->Set(Nan::GetCurrentContext(), JS_STR("height"), JS_INT(height)).ToChecked();
			info.GetReturnValue().Set(arr);
		}
		return;
	}
	...
	NAN_METHOD(SetWindowShouldClose) {
		Nan::HandleScope scope;
		uint64_t handle = info[0]->IntegerValue(Nan::GetCurrentContext()).ToChecked();
		int value = info[1]->Uint32Value(Nan::GetCurrentContext()).ToChecked();
		if (handle) {
			GLFWwindow *window = reinterpret_cast<GLFWwindow *>(handle);
	...
	NAN_METHOD(GetWindowAttrib) {
		Nan::HandleScope scope;
		uint64_t handle = info[0]->IntegerValue(Nan::GetCurrentContext()).ToChecked();
		int attrib = info[1]->Uint32Value(Nan::GetCurrentContext()).ToChecked();
		if (handle) {
			GLFWwindow *window = reinterpret_cast<GLFWwindow *>(handle);
	...
	NAN_METHOD(GetKey) {
		Nan::HandleScope scope;
		uint64_t handle = info[0]->IntegerValue(Nan::GetCurrentContext()).ToChecked();
		int key = info[1]->Uint32Value(Nan::GetCurrentContext()).ToChecked();
		if (handle) {
			GLFWwindow *window = reinterpret_cast<GLFWwindow *>(handle);
	...
	NAN_METHOD(GetMouseButton) {
		Nan::HandleScope scope;
		uint64_t handle = info[0]->IntegerValue(Nan::GetCurrentContext()).ToChecked();
		int button = info[1]->Uint32Value(Nan::GetCurrentContext()).ToChecked();
		if (handle) {
			GLFWwindow *window = reinterpret_cast<GLFWwindow *>(handle);
	...
	NAN_METHOD(GetCursorPos) {
		...
		if (handle) {
			...
			Local<Array> arr = Nan::New<Array>(2);
			arr->Set(Nan::GetCurrentContext(), JS_STR("x"), JS_NUM(x)).ToChecked();
			arr->Set(Nan::GetCurrentContext(), JS_STR("y"), JS_NUM(y)).ToChecked();
			info.GetReturnValue().Set(arr);
		}
		return;
	}
	...
	NAN_METHOD(SetCursorPos) {
		Nan::HandleScope scope;
		uint64_t handle = info[0]->IntegerValue(Nan::GetCurrentContext()).ToChecked();
		int x = info[1]->NumberValue(Nan::GetCurrentContext()).ToChecked();
		int y = info[2]->NumberValue(Nan::GetCurrentContext()).ToChecked();
		if (handle) {
			GLFWwindow *window = reinterpret_cast<GLFWwindow *>(handle);
	...
	NAN_METHOD(SwapInterval) {
		Nan::HandleScope scope;
		int interval = info[0]->Int32Value(Nan::GetCurrentContext()).ToChecked();
	...
	NAN_METHOD(ExtensionSupported) {
		Nan::HandleScope scope;
		String::Utf8Value str(Isolate::GetCurrent(),info[0]->ToString(Nan::GetCurrentContext()).ToLocalChecked());
		info.GetReturnValue().Set(JS_BOOL(glfwExtensionSupported(*str) == 1));
	}

	// make sure we close everything when we exit
	void AtExit() { glfwTerminate(); }
...
//
///////////////////////////////////////////////////////////////////////////////
#define JS_GLFW_CONSTANT(name) target->Set(Nan::GetCurrentContext(), JS_STR(#name), JS_INT(GLFW_##name)).ToChecked()
...
extern "C" {
	NAN_MODULE_INIT(init) {
		 ...
		 * 53-60), but re-arranged to map to 7-bit ASCII for printable keys (function
		 * keys are put in the 256+ range).
		 *
